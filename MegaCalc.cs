	...
	class MegaCalc
	{
		...
		{//функция которая считает ответ выражения
			...
			*/
			...
			if (выр[0] != '-') znak1 = ПозицияЗнака(выр, 1, выр.Length); //если первый знак не минус
			znak2 = ПозицияЗнака(выр, znak1, выр.Length);
			число1 = Число(znak1, znak2, выр);
			return число1;
		}
		static int ПозицияЗнака(string выражение, int откуда, int до_куда)
        {
			int znak = -1;
			bool brek = false; //булевая переменная для выхода из второго цикла
			for (int i = откуда; i < до_куда; i++)
			{//проход по массиву выражения со второго символа, т.к. на первом месте может быть только минус (из функции "нажатие_клавиш")
				for (int j = 0; j < массивЗнаков.Length; j++)
				{//проход по массиву знаков
					if (выражение[i] == массивЗнаков[j]) // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!здесь не работает
					{//если нашёлся знак
						znak = выражение[i]; //сохранение первого знака
